...
router.get('/register', (req, res, next) => {
    res.render('register', { title: 'Register | TechStar Network' });
});

router.get('/login', (req, res, next) => {
    res.render('login', { title: 'Login | TechStar Network' });
});
...
router.get('/posts/create', (req, res, next) => {
    if(req.session.user) {
        res.render('create', { title: 'New Post | TechStar Network' });
    } else {
        res.redirect('/login');
    }    
});
...
router.get('/posts/:id', async (req, res, next) => {
    try {
        ...
        res.render('post', { 
            title: `${user.username} | TechStar Network`,
            post: post,
            postUser: user
        });
    } catch(err) {
...
router.get('/profile/:username', async (req, res, next) => {
    ...
    try {
        ...
        if(user) {
            ...
            res.render('profile',{
                title: `${username} | TechStar Network`,
                currentUser: user,
                posts: userPosts
            });
        } else {
...
router.post('/register', async (req, res, next) => {
    ...
    if(errors.length > 0) {
        ...
        try {
            await newUser.save();
            await mailer.send({ to: email, subject: 'Welcome to TechStar Network', body: `Welcome ${username}!` });
        } catch(err) {
            res.json({ error: err }); 
        }
